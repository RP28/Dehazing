...
def conv_block(in_nc, out_nc, kernel_size, stride=1, dilation=1, groups=1, bias=True, \
               pad_type='zero', norm_type=None, act_type='lrelu', mode='CNA'):
    '''
...
class BB (nn.Module) :
  def __init__(self, nf, splitfactors=4, heads=8, k=3) :
    ...
    self.lk3_1 = conv_block(in_nc=nf, out_nc=nf, kernel_size= 5)
    self.lk3_2 = conv_block(in_nc=2*nf, out_nc=nf, kernel_size= 5)
    self.lk3_3 = conv_block(in_nc=3*nf, out_nc=nf, kernel_size= 5)
    self.k1 = conv_block(in_nc=4*nf, out_nc=nf, kernel_size= 1)
    self.k2 = conv_block(in_nc=4*nf, out_nc=nf, kernel_size= 1)
...
class CB (nn.Module) :
  def __init__(self, nf, no_bb) :
    ...
    self.bb = BB(nf)
    self.fwd_bb = nn.ModuleList([BB(nf) for i in range(no_bb-1)])

  def forward(self,x) :
    x1 = self.bb(x)
    for l in self.fwd_bb :
      x1 = l(x1)
...
class OurUpSample(nn.Module):
    def __init__(self,in_nc, kernel_size=3, stride=1, bias=True, pad_type='zero', \
        ...
        self.co1 = conv_block(in_nc, in_nc, kernel_size=1, norm_type=None, act_type='prelu', mode='CNA')
...
class DualReconstruct(nn.Module) :
  def __init__(self,in_nc, kernel_size=3, upscale_factor=2):
    super(DualReconstruct, self).__init__() 
    self.ps1 = pixelshuffle_block(in_nc, in_nc, upscale_factor=upscale_factor, kernel_size=3, norm_type = 'batch')
    self.ps2 = pixelshuffle_block(in_nc, in_nc, upscale_factor=upscale_factor, kernel_size=3, norm_type = 'batch')
    self.c1 = conv_block(in_nc, in_nc, kernel_size=1, norm_type=None, act_type='prelu', mode='CNA')
    self.c2 = conv_block(in_nc, in_nc, kernel_size=1, norm_type=None, act_type='prelu', mode='CNA')   

    self.us = nn.Upsample(scale_factor=2, mode="bicubic")
    self.dc = nn.ConvTranspose2d(in_nc, in_nc, kernel_size=2, stride=2)

  def forward(self, x) :
    # Convolutional -> Pixel Shuffle -> Convolutional -> Pixel Shuffle
    x1 = self.ps1(x)
    x1 = self.ps2(x1)

    # Bicubic Upsample -> Transpose Conv
...
class MyNetwork (nn.Module) :
  def __init__(self, nf, no_cb, no_bb, in_c = 3) :
    ...
    self.u = nn.Sequential(OurUpSample(nf, kernel_size=3, act_type='leakyrelu',upscale_factor=2),OurUpSample(nf, kernel_size=3, act_type='leakyrelu',upscale_factor=2))
    self.k3 = conv_block(in_nc=nf, out_nc=nf, kernel_size= 3)
    self.k3_1 = conv_block(in_nc=nf, out_nc=in_c, kernel_size= 3)
    self.drm = DualReconstruct(nf)
