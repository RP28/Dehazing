...
class UDB(nn.Module) :
  def __init__(self, nf, ns=5, upscale_factor=2, downscale_factor=0.5):
    super(UDB, self).__init__()
    self.ns = ns
    self.k3_0 = conv_block(nf, nf, 3)
    self.k3_1 = conv_block(nf, nf, 3)
    self.k3 = nn.ModuleList([conv_block(2*nf, nf, 3) for i in range(2*ns)])
    self.k1 = conv_block(nf*3, nf, 1)
    self.T = Transformer(nf)
    self.down = nn.Upsample(scale_factor=downscale_factor)
    self.up = nn.Upsample(scale_factor=upscale_factor)
    

  def slice(self, index, x_top, x_up, x_down, x_bottom) :
    x_top = self.k3[index](torch.cat((x_top, x_down), dim=1))
    x_bottom = self.k3[self.ns//2 + index](torch.cat((x_bottom, x_up), dim=1))
    x_up = self.up(x_top)
    x_down = self.down(x_bottom)


    # x_top = self.k3[index](x_top + x_down)
    # x_bottom = self.k3[self.ns//2 + index](x_bottom + x_up)
    # x_up = self.up(x_top)
    # x_down = self.down(x_bottom)

    return x_top, x_up, x_down, x_bottom


  def forward(self, x) :
    
    x_top = self.down(x)
    x_transformer = self.T(x)
    x_bottom = self.k3_0(x)
    x_top = self.k3_1(x_top)
    x_up = self.up(x_top)
    x_down = self.down(x_bottom)

    for i in range(self.ns) :
        x_top, x_up, x_down, x_bottom = self.slice(i, x_top, x_up, x_down, x_bottom)
    
    x_top = self.up(x_top)
    x_cat = torch.cat((x_top, x_bottom, x_transformer), dim=1)
    x_f = self.k1(x_cat)
...
class CB (nn.Module) :
  def __init__(self, nf, no_bb, type="UDB") :
    super(CB, self).__init__()
    self.no_bb = no_bb
    self.bb = nn.ModuleList([UDB(nf) for _ in range(no_bb)])
